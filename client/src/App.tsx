...
import { Trade } from '../../common/src/types'
...
export default function App() {
	...
	const secondLastPrice = useMemo(
		() => sortedMessageHistory[sortedMessageHistory.length - 2]?.p,
		[sortedMessageHistory]
	)
	...
	const priceChangeSince5MinutesAgoPercentage = useMemo(
		() => priceChangeSinceXPercentage(sortedMessageHistory[sortedMessageHistory.length - 1]?.p, 5),
		[sortedMessageHistory, priceChangeSinceXPercentage]
	)

	const priceChangeSince30MinutesAgoPercentage = useMemo(
		() => priceChangeSinceXPercentage(sortedMessageHistory[sortedMessageHistory.length - 1]?.p, 10),
		[sortedMessageHistory, priceChangeSinceXPercentage]
	)

	const priceChangeSince1HourAgoPercentage = useMemo(
		() => priceChangeSinceXPercentage(sortedMessageHistory[sortedMessageHistory.length - 1]?.p, 60),
		[sortedMessageHistory, priceChangeSinceXPercentage]
	)
