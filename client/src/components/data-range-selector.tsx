interface DateRangeSelectorProps {
	readonly dates: Date[]
	readonly onDateChange: (from: Date, to: Date) => void
}

export function DateRangeSelector({ dates, onDateChange }: DateRangeSelectorProps) {
	...
	return (
		<div className='flex items-center justify-center gap-4 mt-4'>
			...
			/>
			<button onClick={() => onDateChange(new Date(Date.now() - DAY_IN_MS), new Date(Date.now() + DAY_IN_MS))}>
				Today
			</button>
